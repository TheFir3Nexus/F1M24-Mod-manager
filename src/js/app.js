...
const Home = {
  ...
  renderTeam() {
    ...
    const TEAM = [
      ...
      { name: 'lcskpmn',          id: '2759987', url: 'https://www.overtake.gg/members/lcskpmn.2759987/' },
    ];
    // ─────────────────────────────────────────────────────────────────────
...
App.init();
setTimeout(async () => {
  // 1. App update check — runs first, blocks mod check if update found
  await AppUpdate.check(true);
  // 2. Mod update check — only runs if no app update modal is open
  const appUpdateOpen = document.getElementById('app-update-modal')?.style.display !== 'none';
  if (!appUpdateOpen) App.checkForUpdates();
}, 3000); // 3s — slightly faster so update prompt appears promptly
...
const AppUpdate = {
  ...
  async check(silent = true) {
    ...
    try {
      ...
      if (!info) {
        if (!silent) toast('Already on the latest version ✓', 'ok');
        return;
      }
      this._info = info;
      this._showBadge();
      ...
      if (btn) btn.textContent = 'Check for updates';
      if (!silent) toast('Update check failed: ' + e.message, 'err');
    }
  },
  ...
  showModal() {
    ...
    document.getElementById('app-current-ver').textContent = this._info.current;
    document.getElementById('app-latest-ver').textContent  = this._info.latest;
    // Render changelog — strip markdown headers for cleanliness
    const notes = (this._info.body || 'No changelog provided.')
      .replace(/#{1,3} /g, '')
      .trim()
      .slice(0, 1200);
    document.getElementById('app-update-notes').textContent = notes;
    document.getElementById('app-update-modal').style.display = 'flex';
  },

  later() {
    document.getElementById('app-update-modal').style.display = 'none';
    // Badge stays visible so they can come back to it
  },
  ...
  async install() {
    ...
    try {
      if (this._info.assetName && this._info.downloadUrl !== this._info.htmlUrl) {
        // Has a direct installer asset — download and run
        await window.api.installAppUpdate(this._info.downloadUrl, this._info.assetName);
        // App will quit and installer will run
      } else {
        // No installer asset — open releases page
        await window.api.openExternal(this._info.htmlUrl);
        document.getElementById('app-update-modal').style.display = 'none';
      }
    } catch (e) {
